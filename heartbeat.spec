...
%if %{is_rhel}
	%define is_centos %(Z=`rpm -q --whatprovides /etc/redhat-release`;A=`rpm -q --qf '%{N}' $Z`;if [ "$A" == "centos-release" ]; then echo 1; else echo 0; fi)
%endif
%if %{is_fedora}
	%undefine is_rhel
	%undefine is_centos
...
%if %{is_centos}
        ...
        %undefine       SSLeay
        %undefine pkg_group
        %define PILS_NAME       heartbeat-pils
        %define STONITH_NAME    heartbeat-stonith
        %define SSLeay  perl-Net_SSLeay
        %define pkg_group       Productivity/Clustering/HA
%endif
%if %{is_fedora}
...
%if %{is_centos}
BuildRequires: e2fsprogs-devel, swig, pam-devel 
%endif
...
Requires: perl-MailTools, perl-Net-IMAP-Simple, perl-Net-IMAP-Simple-SSL, perl-POP3Client
...
Requires: perl-LDAP
%else
Requires: perl-ldap perl-libnet
%endif
...
%dir %{HA_COREDIR}
%dir %attr (0700, root, root) %{HA_COREDIR}/root
%dir %attr (0700, %{NOUSER}, %{NOUSER}) %{HA_COREDIR}/%{NOUSER}
%dir %attr (0700, %{HA_CCMUSER}, %{HA_APIGROUP}) %{HA_COREDIR}/%{HA_CCMUSER}
...
- added detection script for is_centos
- added libnet to the LIBNET define for is_centos for CentOS
- added buildrequires for e2fsprogs-devel, swig, pam-devel to heartbeat for is_centos
- modified attrs that had attr($user, -) to also have a group.
- modified package {LDIR_NAME} to remove perl-ldap and perl-libnet and add perl-LDAP for CentOS
...
+ Version 2.1.1 - bug fixes and enhancements - changelog includes everything after 2.0.8
  ...
  + Remove CVS artifacts. ($Id: heartbeat.spec,v 1.1 2008-04-18 11:58:01 dhrbac Exp $ and $Log: not supported by cvs2svn $ have no meaning to mercurial.)
